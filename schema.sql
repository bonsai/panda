...
BEGIN
    ...
    SELECT
        NEW.x, NEW.y,
        COALESCE((SELECT CASE stone WHEN 'B' THEN 'W' ELSE 'B' END
...
fwd(dx, dy, steps) AS (
    ...
    UNION ALL
    SELECT f.dx, f.dy, f.steps + 1
    FROM fwd f, last_stone ls, gomoku_board b
    WHERE b.x = ls.x + f.dx * (f.steps + 1)
      AND b.y = ls.y + f.dy * (f.steps + 1)
      AND b.stone = ls.stone AND f.steps < 4
),
rev(dx, dy, steps) AS (
    SELECT dx, dy, 0 FROM dirs
    UNION ALL
    SELECT r.dx, r.dy, r.steps + 1
    FROM rev r, last_stone ls, gomoku_board b
    WHERE b.x = ls.x - r.dx * (r.steps + 1)
      AND b.y = ls.y - r.dy * (r.steps + 1)
